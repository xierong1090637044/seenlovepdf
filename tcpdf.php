...
class TCPDF {
	...
	public function Footer() {
		...
		if (empty($this->pagegroups)) {
			$pagenumtxt = $w_page.$this->getAliasNumPage().'页， 共'.$this->getAliasNbPages().'页';
		} else {
		...
		if ($this->getRTL()) {
			$this->SetX($this->original_rMargin);
			$this->Cell(0, 0, $pagenumtxt, 1, 0, 'R');
		} else {
			$this->SetX($this->original_lMargin);
			$this->Cell(0, 0, $this->getAliasRightShift().$pagenumtxt, 1, 0, 'R',false,'',0,false,'T','R');
		}
	}
	...
	public function setSpotColor($type, $name, $tint=100) {
		...
		$thiscolor = sprintf('/CS%d ', $this->spot_colors[$name]['i']);
		switch ($type) {
			case 'draw': {
				$thiscolor .= sprintf('CS %F SCN', $tint);
				$this->DrawColor = $thiscolor;
	...
	public function setColor($type, $col1=0, $col2=-1, $col3=-1, $col4=-1, $ret=false, $name='') {
		...
		if (($col2 == -1) AND ($col3 == -1) AND ($col4 == -1)) {
			...
			$thiscolor = sprintf('%F %F %F ', ($col1 / 255), ($col2 / 255), ($col3 / 255));
			$suffix = 'rg';
		} else {
			...
			if (empty($name)) {
				...
				$thiscolor = sprintf('%F %F %F %F ', ($col1 / 100), ($col2 / 100), ($col3 / 100), ($col4 / 100));
				$suffix = 'k';
			} else {
				// SPOT COLOR
				$intcolor = array('C' => $col1, 'M' => $col2, 'Y' => $col3, 'K' => $col4, 'name' => $name);
				$this->AddSpotColor($name, $col1, $col2, $col3, $col4);
				$thiscolor = $this->setSpotColor($type, $name, 100);
			}
		}
		switch ($type) {
			case 'draw': {
				$thiscolor .= strtoupper($suffix);
				$this->DrawColor = $thiscolor;
	...
	public function FunctionSet($title="")
	{
		...
		$this->setPrintHeader(true); //设置打印页眉
		$this->setPrintFooter(true); ///设置打印页脚
	}

	public function BannerPage($company,$time,$pdfname,$produceby,$bannerimg,$companyimg)
	{
		$this->AddPage();
		$this->Ln(100);
		$this->SetFont('simhei', '', 14, '', true);

		$this->Image($bannerimg, 0, 40, 50, 50, 'PNG', '', '', true,300,'C');
		$this->Cell(0, 0, $company, 0, 1, 'C', 0, '', 0);
		$this->Ln(10);
		$this->SetFont('simhei', '', 17, '', true);
		$this->Cell(0, 0, $time, 0, 1, 'C', 0, '', 0);
		$this->SetFont('simhei', '', 30, '', true);
		$this->Text(20, 150, $pdfname,false,false,true,0,0,'C');
		$this->Image($companyimg, 0, 220, 20, 20, 'PNG', '', '', true,300,'C');
		$this->SetFont('simhei', '', 15, '', true);
		//$this->Cell(0, 100, '本报告由芯乐提供', 0, 1, 'C', 0, '', 0);
		$this->Text(20, 240, $produceby,false,false,true,0,0,'C');
		$this->AddPage();
	}

/*	public function AddRadarImage($data)
	{
			$h =70;
			$img = new RadarChart($data);
			$this->Ln(5);
			$this->Image($img->aradarImg(), '', '', 100, $h, 'PNG', '', '', true,300,'C');
			return $h;
	}

	public function AddRingImage($data1,$data2)
	{
		$h =70;
		$img = new RingChart($data1,$data2);
		$this->Ln(5);
		$this->Image($img->ringImg(), '', '', 100, $h, 'PNG', '', '', true,300,'C');
		return $h;
	}

	public function AddOdotutexImage($data)
	{
		$h =40;
		$img = new OdotutexChart($data);
		$this->Ln(5);
		$this->Image($img->odotutexImg(), '', '', 50, $h, 'PNG', '', '', true,300,'C');
		return $h;
	}

	public function AddBarchartImage($data1,$data2,$data3)
	{
		$h =60;
		$img = new BarChart($data1,$data2,$data3);
		$this->Ln(5);
		$this->Image($img->barchartImg(), '', '', 140, $h, 'PNG', '', '', true,300,'C');
		return $h;
	}*/
	...
	{
		$this->Ln(5);
		$this->SetFont('simhei', '', 12,'',false);
		$this->MultiCell(0,0,$text);
	}

	public function RadarImage($file)
	{
		$h =70;
		$this->Ln(5);
		$this->Image($file, '', '', 100, $h, 'PNG', '', '', true,300,'C');
		return $h;
	}

	public function OdotutexImage($file)
	{
		$h =40;
		$this->Ln(5);
		$this->Image($file, '', '', 50, $h, 'PNG', '', '', true,300,'C');
		return $h;
	}

	public function BarchartImg($file)
	{
		$h =60;
		$this->Ln(5);
		$this->Image($file, '', '', 140, $h, 'PNG', '', '', true,300,'C');
		return $h;
	}

	public function AddRadarImage($img)
	{
		$this->Ln($this->RadarImage($img->aradarImg()));
	}

	public function AddRingImage($img)
	{
		$this->Ln($this->RadarImage($img->ringImg()));
	}

	public function AddOdotutexImage($img)
	{
		$this->Ln($this->OdotutexImage($img->odotutexImg()));
	}

	public function AddBarchartImage($img)
	{
		$this->Ln($this->BarchartImg($img->barchartImg()));
	}
	 ...
	 *  // - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
	 *  $this->MultiCell($w=0, $h=0, $txt, $border=1, $align='L', $fill=false, $ln=1, $x='', $y='', $reseth=true, $stretch=0, $ishtml=false, $autopadding=true, $maxh=0);
	 *  // - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
	 *  // get the new Y
	 *  $end_y = $this->GetY();
	...
	public function Output($name='doc.pdf', $dest='I') {
		...
		if ($this->sign) {
			...
			$thisdoc = $this->getBuffer();
			// remove last newline
			$thisdoc = substr($thisdoc, 0, -1);
			// remove filler space
			$byterange_string_len = strlen(TCPDF_STATIC::$byterange_string);
			// define the ByteRange
			$byte_range = array();
			$byte_range[0] = 0;
			$byte_range[1] = strpos($thisdoc, TCPDF_STATIC::$byterange_string) + $byterange_string_len + 10;
			$byte_range[2] = $byte_range[1] + $this->signature_max_length + 2;
			$byte_range[3] = strlen($thisdoc) - $byte_range[2];
			$thisdoc = substr($thisdoc, 0, $byte_range[1]).substr($thisdoc, $byte_range[2]);
			// replace the ByteRange
			$byterange = sprintf('/ByteRange[0 %u %u %u]', $byte_range[1], $byte_range[2], $byte_range[3]);
			$byterange .= str_repeat(' ', ($byterange_string_len - strlen($byterange)));
			$thisdoc = str_replace(TCPDF_STATIC::$byterange_string, $byterange, $thisdoc);
			...
			$thisdoc_length = strlen($thisdoc);
			fwrite($f, $thisdoc, $thisdoc_length);
			fclose($f);
			...
			// extract signature
			$signature = substr($signature, $thisdoc_length);
			...
			$this->buffer = substr($thisdoc, 0, $byte_range[1]).'<'.$signature.'>'.substr($thisdoc, $byte_range[1]);
